...
export function NewSession({ trigger }: NewSessionDialogProps) {
  ...
  const onSubmit = async (data: FormData) => {
    if (files.length === 0) {
      toast.error("Please attach at least one TXT file.");
      return;
    }
    ...
    <Dialog open={open} onOpenChange={setOpen}>
      ...
      <DialogContent className="">
        <DialogHeader>
          ...
          <DialogDescription>
            Enter the session details and attach the reports from by the{" "}
            <span className=" text-blue-600 font-semibold">Bituach Leumi</span> main-frame.
          </DialogDescription>
        </DialogHeader>
        <Form {...form}>
