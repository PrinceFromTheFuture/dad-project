...
const sort = (agents: Agent[], sorting: Setting["sorting"]) => {
  return (
    sorting === "name"
      ...
      : agents.sort((a, b) => a.operations.reduce((a, b) => a + b.repeated, 0) - b.operations.reduce((a, b) => a + b.repeated, 0))
  ).map((agent) => {
...
async function generatePDFreport(sorting: Setting["sorting"], reportURL: string) {
  ...
  const cursor = { x: 0, y: CONFIG.dimensions.page.height - CONFIG.dimensions.mainContentTopMargin };
